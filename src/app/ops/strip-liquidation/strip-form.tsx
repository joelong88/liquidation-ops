...
export function StripLiquidationForm() {
  ...
  async function handleLookup(e: React.FormEvent) {
    ...
    if (sackError || !sackRow) {
      setBanner({ ok: false, message: 'No closed Liquidation-area sack with that code — close it first.' })
      setPreview(null)
      setLooking(false)
      return
    }
  ...
  async function handleConfirmStrip() {
    ...
    if (error) {
      ...
      const result = data as { ok: boolean; error?: string; parcels_advanced?: number }
      if (result.ok) {
        setBanner({ ok: true, message: `Stripped — ${result.parcels_advanced} parcel(s) advanced.` })
      } else if (result.error === 'not_found') {
        setBanner({ ok: false, message: 'No closed Liquidation-area sack with that code.' })
